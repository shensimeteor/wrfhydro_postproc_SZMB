...
#MZHU: 2250x2250; 1323x1490+460+310 => 658x734 
...
ln -sf ${consdir}/nodeidx_MZHU.txt .
echo ncl 'srcfilename="??????????00.CHRTOUT_DOMAIN1"' 'colorbar_maxvalue=50' 'dom_name="MZHU"' 'nodeidx_file="nodeidx_MZHU.txt"' 'lonlat_list="113.7877,113.9444,22.6994,22.8472"' plot_streamflow_Customize_forTrans_noLegend2.ncl  
ncl 'srcfilename="??????????00.CHRTOUT_DOMAIN1"' 'colorbar_maxvalue=50' 'dom_name="MZHU"' 'nodeidx_file="nodeidx_MZHU.txt"' 'lonlat_list="113.7877,113.9444,22.6994,22.8472"' plot_streamflow_Customize_forTrans_noLegend2.ncl &> log.plt
...
cd overlayGE
ln -sf $consdir/pngs/MZHU/background.png .
ln -sf $consdir/pngs/MZHU/legend.png .
for png in $(ls ../before_overlay/*.png); do
    convert +repage -transparent "rgb(255,255,255)" -crop 1323x1490+460+310  -resize 658x734! $png inter.png
    ...
    echo cp GE_${pngtitle}.png $webdir/cycles/$cycle/$datx/MZHU_Streamflow.png
    cp GE_${pngtitle}.png $webdir/cycles/$cycle/$datx/MZHU_Streamflow.png
    test -d "$webdir/gifs/$datx" || mkdir -p "$webdir/gifs/$datx" 
    test -e "$webdir/gifs/$datx/MZHU_Streamflow.png" && rm -rf "$webdir/gifs/$datx/MZHU_Streamflow.png"
    echo cp GE_${pngtitle}.png $webdir/gifs/$datx/MZHU_Streamflow.png
    cp GE_${pngtitle}.png $webdir/gifs/$datx/MZHU_Streamflow.png
    cp inter.png $webdir/gifs/$datx/MZHU_Streamflow_White.png
   rm inter.png inter2.png
done
touch $workdir/finished.streamflow_MZHU
