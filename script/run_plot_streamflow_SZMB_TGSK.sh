...
#TGSK: 2250x2250; 1350x1310+450+400 => 606x575 
...
ln -sf ${consdir}/nodeidx_TGSK.txt .
echo ncl 'srcfilename="??????????00.CHRTOUT_DOMAIN1"' 'colorbar_maxvalue=50' 'dom_name="TGSK"' 'nodeidx_file="nodeidx_TGSK.txt"' 'lonlat_list="113.8289,113.9888,22.5847,22.7132"' plot_streamflow_Customize_forTrans_noLegend2.ncl  
ncl 'srcfilename="??????????00.CHRTOUT_DOMAIN1"' 'colorbar_maxvalue=50' 'dom_name="TGSK"' 'nodeidx_file="nodeidx_TGSK.txt"' 'lonlat_list="113.8289,113.9888,22.5847,22.7132"' plot_streamflow_Customize_forTrans_noLegend2.ncl &> log.plt
...
cd overlayGE
ln -sf $consdir/pngs/TGSK/background.png .
ln -sf $consdir/pngs/TGSK/legend.png .
for png in $(ls ../before_overlay/*.png); do
    convert +repage -transparent "rgb(255,255,255)" -crop 1350x1310+450+400  -resize 606x575! $png inter.png
    ...
    echo cp GE_${pngtitle}.png $webdir/cycles/$cycle/$datx/TGSK_Streamflow.png
    cp GE_${pngtitle}.png $webdir/cycles/$cycle/$datx/TGSK_Streamflow.png
    test -d "$webdir/gifs/$datx" || mkdir -p "$webdir/gifs/$datx" 
    test -e "$webdir/gifs/$datx/TGSK_Streamflow.png" && rm -rf "$webdir/gifs/$datx/TGSK_Streamflow.png"
    echo cp GE_${pngtitle}.png $webdir/gifs/$datx/TGSK_Streamflow.png
    cp GE_${pngtitle}.png $webdir/gifs/$datx/TGSK_Streamflow.png
    cp inter.png $webdir/gifs/$datx/TGSK_Streamflow_White.png
    rm inter.png inter2.png
done
touch $workdir/finished.streamflow_TGSK
